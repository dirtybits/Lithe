      ...
      if (m_currency.upgradeHeight(m_targetVersion) != UNDEF_HEIGHT) {
        ...
        if (m_blockchain.size() <= upgradeHeight()) {
          ...
          if (m_blockchain.size() % (60 * 60 / m_currency.difficultyTarget()) == 0) {
            ...
            logger(Logging::TRACE, Logging::BRIGHT_WHITE) << "###### UPGRADE is going to happen after block index " << upgradeHeight() << " at about " <<
          ...
          logger(Logging::TRACE, Logging::BRIGHT_WHITE) << "###### UPGRADE has happened! Starting from block index " << (upgradeHeight() + 1) <<
        ...
        if (isVotingComplete(lastBlockHeight)) {
          ...
          logger(Logging::TRACE, Logging::BRIGHT_WHITE) << "###### UPGRADE voting complete at block index " << m_votingCompleteHeight <<
